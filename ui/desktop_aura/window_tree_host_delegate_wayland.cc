...
void WindowTreeHostDelegateWayland::OnRootWindowClosed(unsigned handle) {
  ...
  if (!current_active_window_ || GetWindowHandle(current_active_window_->window_) != handle ||
        !open_windows_) {
     return;
  }
...
unsigned WindowTreeHostDelegateWayland::GetWindowHandle(gfx::AcceleratedWidget widget) {
